    ...
    .mutation(({ ctx, input }) =>
      runProcedure(
        Effect.gen(function* () {
          ...
          const version = yield* attempt(() => requireScheduleDraft(ctx, input.versionId));
          if (version.projectId !== input.projectId) {
            return yield* fail("BAD_REQUEST", ctx.t.schedule.baselineNotThisProject);
          }
          if (input.endDate < input.startDate) {
            return yield* fail("BAD_REQUEST", ctx.t.schedule.endBeforeStart);
          }
          if (input.scheduleStart && input.scheduleStart < input.startDate) {
            return yield* fail("BAD_REQUEST", ctx.t.schedule.reportingBeforeProject);
          }
          if (input.periodType === "custom" && !input.periodLengthDays) {
            return yield* fail(
              "BAD_REQUEST",
              interpolate(ctx.t.schedule.customCadenceRange, {
                min: CUSTOM_PERIOD_MIN_DAYS,
                max: CUSTOM_PERIOD_MAX_DAYS,
              }),
            );
          ...
          const [[recorded], [activeSchedule]] = yield* Effect.all([
            attempt(() =>
              db
          ...
          if (recorded || activeSchedule) {
            return yield* fail("CONFLICT", ctx.t.schedule.timingFixed);
          }

          yield* attempt(() =>
            runScheduleDraftMutation(ctx.t, input.projectId, input.versionId, [
      ...
      runProcedure(
        Effect.gen(function* () {
          ...
          const [target] = yield* attempt(() =>
            db.select().from(project).where(eq(project.id, input.projectId)),
          );
          if (!target) {
          ...
          if (!start || !finish) {
            return yield* fail("BAD_REQUEST", ctx.t.schedule.needsProjectDates);
          }
          ...
          if (recorded) {
            return yield* fail("CONFLICT", ctx.t.schedule.progressBlocksRebuild);
          }

          const [existingReport] = yield* attempt(() =>
            db
          ...
          if (existingReport) {
            return yield* fail("CONFLICT", ctx.t.schedule.historyBlocksRebuild);
          }

          const [existingDailyProgress] = yield* attempt(() =>
            db
          ...
          const [linkedAction] = yield* attempt(() =>
            db
              .select({ id: ticket.id })
              .from(ticket)
          ...
          if (linkedAction) {
            return yield* fail("CONFLICT", ctx.t.schedule.actionsBlockRebuild);
          }

          const [activeSchedule] = yield* attempt(() =>
            db
              .select({ id: boqVersion.id })
              .from(boqVersion)
              .where(
          ...
          if (activeSchedule) {
            return yield* fail("CONFLICT", ctx.t.schedule.activeScheduleBlocksRebuild);
          }

          const periods = yield* attemptSync(() => {
            try {
              return generatePeriods(start, finish, target.periodType, target.periodLengthDays);
            } catch (error) {
          ...
          if (periods.length === 0) {
            return yield* fail("BAD_REQUEST", ctx.t.schedule.noPeriodsInDates);
          }
    ...
    .mutation(({ ctx, input }) =>
      runProcedure(
        Effect.gen(function* () {
          ...
          yield* attempt(() => assertLeavesOfVersion(ctx.t, input.versionId, itemIds));
          yield* attempt(() => assertPeriodsOfProject(ctx.t, version.projectId, periodIds));
          ...
          yield* attempt(() =>
            runScheduleDraftMutation(ctx.t, version.projectId, input.versionId, statements),
          );
    ...
    .mutation(({ ctx, input }) =>
      runProcedure(
        Effect.gen(function* () {
          ...
          const periods = yield* attempt(() => listPeriodsFor(version.projectId));
          if (periods.length === 0) {
            return yield* fail("BAD_REQUEST", ctx.t.schedule.generatePeriodsFirst);
          }
          ...
          const windows = yield* attemptSync(() =>
          ...
          if (input.mode === "even") {
            ...
            if (values.length > MAX_PLAN_CELLS) {
              return yield* fail(
                "BAD_REQUEST",
                plural(ctx.t.schedule.tooManyCellsSpread, values.length),
              );
            }
            if (values.length > 0) {
      ...
      runProcedure(
        Effect.gen(function* () {
          ...
          if (targetIds.length === 0) {
            return yield* fail("BAD_REQUEST", ctx.t.schedule.chooseLineToCopy);
          }
          yield* attempt(() =>
            assertLeavesOfVersion(ctx.t, input.versionId, [input.sourceItemId, ...targetIds]),
          );

          const [[source], sourceCells] = yield* Effect.all([
            attempt(() =>
              db
                .select({
          ...
          if (!source) {
            return yield* fail("NOT_FOUND", ctx.t.boq.lineNotFound);
          }

          const cellCount = sourceCells.length * targetIds.length;
          if (cellCount > MAX_PLAN_CELLS) {
            return yield* fail("BAD_REQUEST", plural(ctx.t.schedule.tooManyCellsCopy, cellCount));
          }
    ...
    .mutation(({ ctx, input }) =>
      runProcedure(
        Effect.gen(function* () {
          ...
          yield* attempt(() =>
            runScheduleDraftMutation(ctx.t, version.projectId, input.versionId, [
              liveScheduleItemsGuard(input.versionId, itemIds),
              db.delete(boqItemDistribution).where(inArray(boqItemDistribution.boqItemId, itemIds)),
              db
                .update(boqItem)

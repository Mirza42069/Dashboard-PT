...
export const noteRouter = router({
    ...
    .mutation(({ ctx, input }) =>
      runProcedure(
        Effect.gen(function* () {
          ...
          if (!created) {
            return yield* fail("INTERNAL_SERVER_ERROR", ctx.t.note.couldNotCreate);
          }
    ...
    .mutation(({ ctx, input }) =>
      runProcedure(
        Effect.gen(function* () {
          ...
          if (!photo) {
            return yield* fail("NOT_FOUND", ctx.t.note.photoNotFound);
          }
          yield* attempt(() => assertNoteWritable(ctx, photo.noteId));

          yield* attempt(() => db.delete(notePhoto).where(eq(notePhoto.id, input.id)));
          return { success: true };
        }),
      ),
    ),
  ...
  delete: companyPermissionProcedure("project:write")
    ...
    .mutation(({ ctx, input }) =>
      runProcedure(
        Effect.gen(function* () {
          yield* attempt(() => assertNoteWritable(ctx, input.id));
          // Bytes live in note_photo rows, so the cascade removes them too.
          yield* attempt(() => db.delete(projectNote).where(eq(projectNote.id, input.id)));
          return { success: true };
        }),
      ),
    ),
